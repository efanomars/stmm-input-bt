...
# File:   uninstall_libstmm-input-gtk-bt.py

# Removes all files installed by the install_libstmm-input-gtk-bt.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-input-gtk-bt.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f             {}/include/stmm-input-gtk-bt".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-gtk-bt.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-gtk-bt.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f       {}/lib/pkgconfig/stmm-input-gtk-bt.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f        {}/share/doc/libstmm-input-gtk-bt".format(sSudo, sInstallDir).split())
		if sInstallDir == "/usr":
			subprocess.check_call("{} rm  -f   /etc/stmm-input-dl/stmm-input-gtk-bt.dlp".format(sSudo).split())
		else:
			subprocess.check_call("{} rm  -f {}/etc/stmm-input-dl/stmm-input-gtk-bt.dlp".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-input-gtk-bt/build".format(sSudo).split())
		subprocess.check_call("{} rm -r -f libstmm-input-gtk-bt/examples/keys-srv/build".format(sSudo).split())
