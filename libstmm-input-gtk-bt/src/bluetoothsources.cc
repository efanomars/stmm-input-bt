...
BlueServerAcceptSource::BlueServerAcceptSource(int32_t nL2capPort)
: Glib::Source()
...
BlueServerAcceptSource::~BlueServerAcceptSource()
{
	if (m_nListenerFD >= 0) {
		::close(m_nListenerFD);
	}
}
sigc::connection BlueServerAcceptSource::connect(const sigc::slot<bool, int32_t, const struct sockaddr_l2&>& oSlot)
{
	if (m_nListenerFD == -1) {
...
bool BlueServerAcceptSource::prepare(int& nTimeout)
{
	nTimeout = -1;

	return false;
}
bool BlueServerAcceptSource::check()
{
	bool bRet = false;
...
bool BlueServerAcceptSource::dispatch(sigc::slot_base* p0Slot)
{
	bool bContinue = true;
...
////////////////////////////////////////////////////////////////////////////////
BlueServerReceiveSource::BlueServerReceiveSource(int32_t nBackendId, int32_t nClientFD)
: Glib::Source()
...
BlueServerReceiveSource::~BlueServerReceiveSource()
{
	if (m_nClientFD >= 0) {
		::close(m_nClientFD);
	}
}
sigc::connection BlueServerReceiveSource::connect(const sigc::slot<bool, int32_t, bool, const KeyPacket&>& oSlot)
{
	if (m_nClientFD < 0) {
		// Error, return an empty connection
		return sigc::connection();
...
bool BlueServerReceiveSource::prepare(int& nTimeout)
{
	nTimeout = -1;

	return false;
}
bool BlueServerReceiveSource::check()
{
	bool bRet = false;
...
bool BlueServerReceiveSource::closeConnection(sigc::slot_base* p0Slot, bool bRemove, KeyPacket& oPkt, const std::string& sErr)
{
	Glib::Source::destroy();
	if (! sErr.empty()) {
		std::cerr << sErr << '\n';
...
bool BlueServerReceiveSource::dispatch(sigc::slot_base* p0Slot)
{
	bool bContinue = true;
