...
BtKeysDevice::BtKeysDevice(std::string sName, const shared_ptr<BtGtkDeviceManager>& refDeviceManager)
: BasicDevice<BtGtkDeviceManager>(sName, refDeviceManager)
{
}
BtKeysDevice::~BtKeysDevice()
{
}
shared_ptr<Device> BtKeysDevice::getDevice() const
{
...
shared_ptr<Capability> BtKeysDevice::getCapability(const Capability::Class& oClass) const
{
...
shared_ptr<Capability> BtKeysDevice::getCapability(int32_t nCapabilityId) const
{
...
std::vector<int32_t> BtKeysDevice::getCapabilities() const
{
	return {KeyCapability::getId()};
}
std::vector<Capability::Class> BtKeysDevice::getCapabilityClasses() const
{
	return {KeyCapability::getClass()};
}
bool BtKeysDevice::onBlueKey(KeyEvent::KEY_INPUT_TYPE eInputType, HARDWARE_KEY eHardwareKey, const shared_ptr<GtkWindowData>& refWindowData)
{
...
void BtKeysDevice::finalizeListener(BtGtkDeviceManager::ListenerData& oListenerData, int64_t nEventTimeUsec)
{
	auto refOwner = getOwnerDeviceManager();
	if (!refOwner) {
		return;
...
void BtKeysDevice::removingDevice()
{
	resetOwnerDeviceManager();
}
void BtKeysDevice::cancelSelectedAccessorKeys()
{
	auto refOwner = getOwnerDeviceManager();
	if (!refOwner) {
		return;
