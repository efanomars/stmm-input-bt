...
public:
	...
	void enable(const shared_ptr<GtkAccessor>& refAccessor, BtGtkDeviceManager* p0Owner) noexcept override
	{
		assert(refAccessor);
...
{
public:
	FakeGtkWindowDataFactory() noexcept
	: Private::Bt::GtkWindowDataFactory()
	{
	}
	std::shared_ptr<Private::Bt::GtkWindowData> create() noexcept override
	{
