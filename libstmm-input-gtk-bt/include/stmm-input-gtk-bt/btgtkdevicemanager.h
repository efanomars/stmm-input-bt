...
protected:
	 ...
	 * @see create()
	 */
	BtGtkDeviceManager(bool bEnableEventClasses, const std::vector<Event::Class>& aEnDisableEventClasses);
...
private:
	...
	bool findWindow(Gtk::Window* p0GtkmmWindow
					, std::vector< std::pair<Gtk::Window*, shared_ptr<Private::Bt::GtkWindowData> > >::iterator& itFind);
	bool hasAccessor(const shared_ptr<Accessor>& refAccessor, bool& bValid
					, std::vector< std::pair<Gtk::Window*, shared_ptr<Private::Bt::GtkWindowData> > >::iterator& itFind);
	void removeAccessor(const std::vector< std::pair<Gtk::Window*, shared_ptr<Private::Bt::GtkWindowData> > >::iterator& itGtkData);
	void cancelDeviceKeys(const shared_ptr<Private::Bt::BtKeysDevice>& refBtKeysDevice);
	void selectAccessor(const shared_ptr<Private::Bt::GtkWindowData>& refData);
	void deselectAccessor();
	void onIsActiveChanged(const shared_ptr<Private::Bt::GtkWindowData>& refWindowData);
