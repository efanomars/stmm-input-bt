...
# File:   uninstall_stmm-input-bttest.py

# Removes all files installed by the install_stmm-input-bttest.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_stmm-input-bttest.py")
	...
	subprocess.check_call("{} rm -r -f {}/bin/stmm-input-bttest".format(sSudo, sDestDir).split())
	subprocess.check_call("{} rm -r -f {}/share/stmm-input-bttest".format(sSudo, sDestDir).split())

	if oArgs.bRemoveBuilds:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f stmm-input-bttest/build".format(sSudo).split())
