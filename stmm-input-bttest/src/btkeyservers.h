...
public:
	 ...
	 */
	BtKeyServers(int32_t n1s28PeriodsAddr, int32_t nL2capPort, const ::bdaddr_t& oExtraAddr, bool bFlush);
	...
	static bool isValidStringAddr(const std::string& oBtAddr);
	static bool isValidAddr(const bdaddr_t& oBtAddr);
	static bool isValidPort(int32_t nL2capPort);
	static bool isValid(const ServerInfo& oInfo);
	static std::string getStringFromAddr(const bdaddr_t& oBtAddr);
	static bdaddr_t getAddrFromString(const std::string& sBtAddr);
	static bdaddr_t getAddrCopy(const bdaddr_t& oBtAddr);
	static bool isEmptyAddr(const bdaddr_t& oBtAddr);
	static bdaddr_t getEmptyAddr();
	static bool equalAddrs(const bdaddr_t& oBtAddr1, const bdaddr_t& oBtAddr2);
	 ...
	 */
	bool isThreadHung() const { return m_bHungThread; }
	...
	static constexpr int32_t s_nDefaultL2capPort = 0x20A1;
private:
	BtKeyServers(int32_t n1s28PeriodsAddr, int32_t nL2capPort, bool bExtraServer, const ::bdaddr_t& oExtraAddr, bool bFlush);
	void addExtraServer(std::vector<ServerInfo>& aServers);

	bool doIntervalTimeout();
	// std::future function
	std::pair<std::string, std::vector<BtKeyServers::ServerInfo>> refreshServers(bool bFlush, int32_t n1s28PeriodsAddr);
