...
BtKeyServers::BtKeyServers() noexcept
...
void BtKeyServers::addExtraServer(std::vector<ServerInfo>& aServers) noexcept
{
...
bool BtKeyServers::isValidPort(int32_t nL2capPort) noexcept
{
...
bool BtKeyServers::isValid(const ServerInfo& oInfo) noexcept
{
...
std::string BtKeyServers::getStringFromAddr(const bdaddr_t& oBtAddr) noexcept
{
	char sName[20];
...
bool BtKeyServers::isValidAddr(const bdaddr_t& oBtAddr) noexcept
{
...
bool BtKeyServers::isValidStringAddr(const std::string& sBtAddr) noexcept
{
	bdaddr_t oAddr;
...
bdaddr_t BtKeyServers::getAddrFromString(const std::string& sBtAddr) noexcept
{
	bdaddr_t oAddr;
	#ifndef NDEBUG
	const auto nRet = 
...
bdaddr_t BtKeyServers::getAddrCopy(const bdaddr_t& oBtAddr) noexcept
{
	bdaddr_t oAddr;
	::bacpy(&oAddr, &oBtAddr);
	return oAddr;
}
bdaddr_t BtKeyServers::getEmptyAddr() noexcept
{
	bdaddr_t oAddr;
	::memset(&oAddr, 0, sizeof(oAddr));
	return oAddr;
}
bool BtKeyServers::isEmptyAddr(const bdaddr_t& oBtAddr) noexcept
{
	static const bdaddr_t s_oEmpty = getEmptyAddr();
	return equalAddrs(s_oEmpty, oBtAddr);
}
bool BtKeyServers::equalAddrs(const bdaddr_t& oBtAddr1, const bdaddr_t& oBtAddr2) noexcept
{
	return (::bacmp(&oBtAddr1, &oBtAddr2) == 0);
}
void BtKeyServers::addLocalAddress() noexcept
{
...
bool BtKeyServers::startRefreshServers() noexcept
{
...
std::pair<std::string, std::vector<BtKeyServers::ServerInfo>> BtKeyServers::refreshServers(bool bFlush, int32_t n1s28PeriodsAddr) noexcept
{
...
bool BtKeyServers::doIntervalTimeout() noexcept
{
...
const std::vector<BtKeyServers::ServerInfo>& BtKeyServers::getServers() noexcept
{
