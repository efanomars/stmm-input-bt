...
{
	std::cout << "Usage: stmm-input-btkb [OPTION]" << '\n';
...
bool evalIntArg(int& nArgC, char**& aArgV, const std::string& sOption1, const std::string& sOption2, int32_t& nVar, int32_t nMin) noexcept
{
	if (aArgV[1] == nullptr) {
		return true;
	}
...
bool evalAddrArg(int& nArgC, char**& aArgV, const std::string& sOption1, const std::string& sOption2, ::bdaddr_t& oBtAddr) noexcept
{
	if (aArgV[1] == nullptr) {
		return true;
	}
...
int btkbMain(int nArgC, char** aArgV) noexcept
{
	...
	BtKeyServers oServers(n1s28Periods, nL2capPort, oExtraAddr, bRefreshFlush);
	{
		...
		const Glib::ustring sAppName = "com.efanomars.stmm-input-btkb";
		const Glib::ustring sWindoTitle = "stmm-input-btkb " + Config::getVersionString();
		//
		Glib::RefPtr<Gtk::Application> refApp = Gtk::Application::create(nArgC, aArgV, sAppName);
		BtkbWindow oWindow(sWindoTitle, oClient, oServers);
...
{
	return stmi::btkbMain(nArgC, aArgV);
}
