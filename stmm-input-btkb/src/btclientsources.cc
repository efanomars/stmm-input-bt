...
PendingWriteSource::PendingWriteSource(int32_t nClientFD) noexcept
: Glib::Source()
...
PendingWriteSource::~PendingWriteSource() noexcept
{
	if (m_nClientFD >= 0) {
		::close(m_nClientFD);
	}
...
int32_t PendingWriteSource::removePoll() noexcept
{
...
sigc::connection PendingWriteSource::connect(const sigc::slot<bool, int32_t, bool>& oSlot) noexcept
{
	if (m_nClientFD < 0) {
...
bool PendingWriteSource::prepare(int& nTimeout) noexcept
{
	nTimeout = -1;

	return false;
}
bool PendingWriteSource::check() noexcept
{
	bool bRet = false;
...
bool PendingWriteSource::dispatch(sigc::slot_base* p0Slot) noexcept
{
	bool bContinue = true;
...
////////////////////////////////////////////////////////////////////////////////
IntervalTimeoutSource::IntervalTimeoutSource(int32_t nInterval) noexcept
...
IntervalTimeoutSource::~IntervalTimeoutSource() noexcept
{
//std::cout << "IntervalTimeoutSource::~IntervalTimeoutSource()" << '\n';
}
sigc::connection IntervalTimeoutSource::connectSlot(const sigc::slot<bool>& oSlot) noexcept
{
	m_oCallback = oSlot;
	const sigc::slot<bool> oNoParamSlot = sigc::mem_fun(this, &IntervalTimeoutSource::callback);
	return connect_generic(oNoParamSlot);
}
bool IntervalTimeoutSource::callback() noexcept
{
	if (!m_oCallback) {
		return false;
	}
