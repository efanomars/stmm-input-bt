...
def testAll(sBuildType, sInstallDir, sSudo, bBuildStatic, bSanitize):
	...
	subprocess.check_call("./scripts/uninstall_stmm-input-bt-all.py -y --installdir {}  {}".format(sInstallDir, sSudo).split())

	subprocess.check_call("./scripts/install_stmm-input-bt-all.py -b {}  -t On  -d On --docs-to-log  --installdir {}  {} {} {}"\
			.format(sBuildType, sInstallDir, sSudo, sStatic, sSanitize).split())


	os.chdir("libstmm-input-gtk-bt/build")
	subprocess.check_call(sSanitizeOptions + " make test", shell=True)
	if os.path.getsize("libstmm-input-gtk-bt_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-input-gtk-bt_doxy.log not empty")
	os.chdir("../..")

	if not bSanitize:
		os.chdir("libstmm-input-gtk-bt/examples/keys-srv")
...
def main():
	...
	print("-----------------------------------")
	print("testall.py (stmm-input-bt) started!")
	print("-----------------------------------")
	...
	print("-------------------------------------------------")
	print("testall.py (stmm-input-bt) finished successfully!")
	print("-------------------------------------------------")
